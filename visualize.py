...
if __name__ == '__main__':
    ...
    parser.add_argument('--visualize_backend', type=str, default='open3d', \
    ...
    for point_cloud_name in input_point_clouds:
        ...
        if args.visualize_backend == 'mayavi':
            ...
            bin_vis.run_visualize(feed_dict['pc'], predictions.astype(np.int32), 'Predictions')
            if label is not None:
                bin_vis = BinVisualizer()
                bin_vis.run_visualize(feed_dict['pc'], feed_dict['targets_mapped'], 'Ground turth')
        else:
            raise NotImplementedError
